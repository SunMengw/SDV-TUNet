...
def calculate_metrics(preds, targets, patient, tta=False):
    ...
    for i, label in enumerate(labels):
        ...
        if np.sum(targets[i]) == 0:
            ...
            print(np.sum(preds[i]), np.sum(targets[i]), "targets", "targets")
            tp = np.sum(l_and(preds[i], targets[i]))
            fp = np.sum(l_and(preds[i], l_not(targets[i])))
            fn = np.sum(l_and(l_not(preds[i]), targets[i]))

            dice = 2 * tp / (2 * tp + fp + fn)
        if np.sum(preds[i]) > 0 and np.sum(targets[i]) > 0:
            hd95 = binary.hd95(preds[i], targets[i])
            print(type(preds[i]), "preds[i]")
        else:
            hd95 = 0
        metrics[DICE] = dice
...
def generate_segmentations_monai(data_loader, model, writer_1, args):
    ...
    grouped_df = df.groupby("label")[METRICS[0]]
    ...
    for metric, label_values in summary.items():
        ...
        writer_1.add_scalar(f"benchmark_{metric}", label_values)
    df.to_csv((args.save_folder_1 / 'results.csv'), index=False)


HAUSSDORF = "haussdorf"
DICE = "dice"
METRICS = [DICE, HAUSSDORF]
