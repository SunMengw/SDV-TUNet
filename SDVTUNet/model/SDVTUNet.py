...
class SparseAttention3D(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads, patch_size, stride, dropout_rate=0.1):
        super(SparseAttention3D, self).__init__()
        self.num_heads = num_heads
        self.patch_size = patch_size
        self.stride = stride

        self.key_conv = nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=1)
        self.query_conv = nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=1)
        self.value_conv = nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=1)
        self.attention_dropout = nn.Dropout(dropout_rate)

        self.unfold = nn.Unfold(kernel_size=(patch_size, patch_size, patch_size), stride=stride, padding=0)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        keys_unfolded = self.unfold(keys)
        queries_unfolded = self.unfold(queries)
        values_unfolded = self.unfold(values)

        keys_unfolded = keys_unfolded.view(B, self.num_heads, -1, keys_unfolded.size(-1))
        queries_unfolded = queries_unfolded.view(B, self.num_heads, -1, queries_unfolded.size(-1))
        values_unfolded = values_unfolded.view(B, self.num_heads, -1, values_unfolded.size(-1))

        attention_scores = torch.matmul(queries_unfolded.transpose(-1, -2), keys_unfolded)
        attention_probs = self.softmax(attention_scores)
        attention_probs = self.attention_dropout(attention_probs)

        output = torch.matmul(attention_probs, values_unfolded.transpose(-1, -2))
        output = output.view(B, -1, D - self.patch_size + 1, H - self.patch_size + 1, W - self.patch_size + 1)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,alpha=0.5,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_checkpoint=False,patch_size=16,stride=1):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
    ...
    def forward_part1(self, x, mask_matrix, prev_v, prev_k, prev_q, is_decoder):
        ...
        # merge attention
        sparse_attn_output = self.attn2(x)  # Apply sparse attention on the entire feature map

        attn_windows = self.alpha * attn_windows + (
                    1 - self.alpha) * sparse_attn_output  # Element-wise combination of attention results
...
class MEFFblock(nn.Module):
    def __init__(self, channel):
        ...
        self.bn1 = normalization(channel, norm='gn')
...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        self.dblock1 = MEFFblock(384)
        self.bn1 = normalization(384, norm='gn')
        self.relu1 = nn.ReLU(inplace=True)
        self.dblock2 = MEFFblock(192)
        self.bn2 = normalization(192, norm='gn')
        self.relu2 = nn.ReLU(inplace=True)
        self.dblock3 = MEFFblock(96)
        self.bn3 = normalization(96, norm='gn')
    ...
    def forward_up_features(self, x, x_downsample, v_values_1, k_values_1, q_values_1, v_values_2, k_values_2,
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                if inx==1:
                    x1 = self.dblock1(x_downsample[3 - inx])
                    x1 = self.bn1(x1)
                    x1 = self.relu1(x1)
                elif inx ==2:
                    x1 = self.dblock2(x_downsample[3 - inx])
                    x1 = self.bn2(x1)
                    x1 = self.relu2(x1)
                else:
                    x1 = self.dblock3(x_downsample[3 - inx])
                    x1 = self.bn3(x1)
                    x1 = self.relu3(x1)
                x = x * x1
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        model = SwinTransformerSys3D(img_size=(128, 128, 128),
                                            patch_size=(4, 4, 4),
                                            in_chans=4,
                                            num_classes=3,
                                            embed_dim=48,
